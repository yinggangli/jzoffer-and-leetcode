...
## JZ66 构建乘积数组 :flushed:
`中等` `题目`<br>
*给定一个数组 A[0,1,…,n-1]，请构建一个数组 B[0,1,…,n-1]，其中 B[i] 的值是数组 A 中除了下标 i 以外的元素的积, 即 B[i]=A[0]×A[1]×…×A[i-1]×A[i+1]×…×A[n-1]。不能使用除法。*<br><br>
`考点`<br>
数组<br><br>
`代码`<br>
```java
class Solution {
    public int[] constructArr(int[] a) {
        ...
        int[] res = new int[n];
        //对于数组中的第i个元素
        //第一个for循环实现的乘积过程：a1 * ...... * ai-1
        for(int i=0, p=1; i<n; i++){
            res[i] = p;
            p *= a[i];
        }
        //第二个for循环实现的乘积过程：ai+1 * ...... * an-1
